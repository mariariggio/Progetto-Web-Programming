...
//funzione che crea il form per l'inserimento di una nuova operazione
...
//funzione che crea il form per l'inserimento di un nuovo articolo
function addArticle() {
    s = "<center><form name='addArticle'><table border=0px>";
    ...
    s += "<tr><td>Quantita</td><td><input type='text' name='quantita'></td></tr>";
    ...
    s += "<tr><td colspan=2 style=text-align:center><br><input type='button' onclick=makeAddArticleJson() value='Salva'></td></tr></table></form></center>";
    $("#post").html(s);
}
...
function makeAddArticleJson() {
    ...
    if (x.codArticolo.value === "" || x.categoria.value === "" || x.descrizione.value === ""
        ...
        request = {
            ...
            "codFornitore": x.codFornitore.value
        };
        //Converto l'oggetto Json in stringa per inviarlo al server.
        req = JSON.stringify(request);
        ajaxEvent(req);
